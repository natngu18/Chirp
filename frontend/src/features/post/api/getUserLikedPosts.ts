...
> => {
    // TODO: implement user likes tab:
    const response = await axiosInstance.get(`users/${username}/posts/likes`, {
...
export const useGetUserLikedPostsInfinite = (
    params: PaginationParams & { username: string }
) => {
    const { token } = useAuth()
    return useInfiniteQuery({
        // queryKey: ['userReplies', params.username],
        queryKey: postQueryKeys.userLikedPosts(params.username),
        queryFn: ({ pageParam = 1 }) =>
            getUserLikedPosts({ ...params, pageNumber: pageParam, token }),
